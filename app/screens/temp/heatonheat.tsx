...
export default function HeatOnHeatScreen({ navigation }: any = {}) {
  const [animalId, setAnimalId] = useState(
    "832ff39b-40e3-45fe-8f3f-cbb84be9d2bf",
  );
  ...
  const [recommendedPickerMode, setRecommendedPickerMode] = useState<
    "date" | "time"
  >("date");
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <TextInput
              style={[
                styles.input,
                animalId === PRESET_VALUES.animalId && styles.inputPrefilled,
              ]}
              placeholder="e.g. C-001"
              value={animalId}
              onChangeText={setAnimalId}
              onFocus={() =>
                animalId === PRESET_VALUES.animalId && setAnimalId("")
              }
              onBlur={() =>
                animalId.trim() === "" && setAnimalId(PRESET_VALUES.animalId)
              }
              returnKeyType="next"
            />
                ...
                {symptomOptions.map((s) => (
                  <View key={s} style={styles.symptomField}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        openSelect(`symptom:${s}`, ["Select", "Yes", "No"])
                      }
                    >
                      <Text
                        style={[
            ...
            <TouchableOpacity
              style={styles.selectInput}
              onPress={() => setShowDateReportedPicker(true)}
            >
              <AppText
                style={[
                  styles.selectText,
                  !dateReportedDate && styles.selectPlaceholder,
                ]}
              >
                {dateReportedDate ? formatDate(dateReportedDate) : "dd-mm-yyyy"}
              </AppText>
              <MaterialCommunityIcons
                name="calendar"
                size={spacing.md}
            ...
            <TouchableOpacity
              style={styles.selectInput}
              onPress={() => openSelect("isLactating", ["Select", "Yes", "No"])}
            >
              <AppText
                style={[
                  styles.selectText,
                  !isLactating && styles.selectPlaceholder,
                ]}
              >
                {isLactating
                  ? isLactating === "yes"
            ...
            <View style={styles.sectionCard}>
              ...
              <TextInput
                style={[
                  styles.input,
                  doctorId === PRESET_VALUES.doctorId && styles.inputPrefilled,
                ]}
                value={doctorId}
                onChangeText={setDoctorId}
                onFocus={() =>
                  doctorId === PRESET_VALUES.doctorId && setDoctorId("")
                }
                onBlur={() =>
                  doctorId.trim() === "" && setDoctorId(PRESET_VALUES.doctorId)
                }
              />
              ...
              <TextInput
                style={[
                  styles.input,
                  doctorName === PRESET_VALUES.doctorName &&
                    styles.inputPrefilled,
                ]}
                value={doctorName}
                onChangeText={setDoctorName}
                onFocus={() =>
                  doctorName === PRESET_VALUES.doctorName && setDoctorName("")
                }
                onBlur={() =>
                  doctorName.trim() === "" &&
                  setDoctorName(PRESET_VALUES.doctorName)
                }
              />
              ...
              <TouchableOpacity
                style={styles.selectInput}
                onPress={() => openSelect("methodConfirmation", methodOptions)}
              >
                <AppText
                  style={[
                    styles.selectText,
                    !methodConfirmation && styles.selectPlaceholder,
                  ]}
                >
                  {methodConfirmation || "Select"}
                </AppText>
                <MaterialCommunityIcons
                  name="chevron-down"
                  size={spacing.md}
              ...
              <TouchableOpacity
                style={styles.selectInput}
                onPress={() => openSelect("finalResult", finalResultOptions)}
              >
                <AppText
                  style={[
                    styles.selectText,
                    !finalResult && styles.selectPlaceholder,
                  ]}
                >
                  {finalResult || "Select"}
                </AppText>
            ...
            >
              <AppText
                style={[
                  styles.selectText,
                  !recommendedAiDate && styles.selectPlaceholder,
                ]}
              >
                {recommendedAiDate
                  ? formatDateTime(recommendedAiDate)
                  : "dd-mm-yyyy --:--"}
              </AppText>
                ...
                onChange={(e, selected) => {
                  ...
                  if (recommendedPickerMode === "date") {
                    ...
                    if (recommendedAiDate) {
                      base.setHours(
                        recommendedAiDate.getHours(),
                        recommendedAiDate.getMinutes(),
                      );
                    }
                    ...
                    const dt = recommendedAiDate
                      ? new Date(recommendedAiDate)
                      : new Date();
                    dt.setHours(
                      (selected as Date).getHours(),
                      (selected as Date).getMinutes(),
                    );
            ...
            <View style={styles.actions}>
              ...
              >
                {submitting ? (
                  ...
                  <AppText style={styles.submitText}>Submit Heat Form</AppText>
                )}
              </TouchableOpacity>
            </View>
          </View>
