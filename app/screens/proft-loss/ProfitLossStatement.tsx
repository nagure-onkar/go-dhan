...
const ProfitLossStatement = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/v1/profit/daily/compute`, {
        method: "POST",
        headers: {
  ...
  const Section = ({ title, style }: { title: string; style?: any }) => (
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          ...
          <Row>
            ...
            <Input
              ...
              onChangeText={(text) => setSales({ ...sales, calfSales: text })}
            />
            <Input
              label="Cattle Sales"
              value={sales.cattleSales}
              onChangeText={(text) => setSales({ ...sales, cattleSales: text })}
            />
          </Row>
